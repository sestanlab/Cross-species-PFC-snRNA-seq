...
CirclePlot.vertical <- function(avg, ratio, features, file_name, dot.min = 0, dot.scale = 4, scale.by = "radius", shape = 16, cluster.order = NULL, stroke.col = "black", col.min = -2.5, col.max = 2.5, scale = TRUE, stroke.size = 0.5, stroke.matrix = NULL, scale.min = NA, scale.max = NA, return.plot = FALSE, width.scale = 1, height.base = 1.5, col.cex.sf = 1, row.cex.sf = 1, height.unit = 0.6, split.order = c("Human", "Chimpanzee", "Rhesus", "Marmoset")){
    cols <- c("#FF420E", "#4CB5F5", "#89DA59", "#FFBB00", "#AE017E") %>% setNames(., c("Human", "Chimpanzee", "Rhesus", "Marmoset", "Mouse"))
    ngradient <- 20
    ...
    plot <- plot +
            ...
            theme(axis.title.x = element_blank(), axis.title.y = element_blank(), axis.text.y=element_blank(), axis.text.x=element_text(size = rel(0.8 * col.cex.sf)), strip.background = element_blank(), strip.text = element_text(size = rel(1 * row.cex.sf), angle = 0), strip.text.y.left = element_text(size = rel(1 * row.cex.sf), angle = 0), strip.placement = 'outside', panel.spacing = unit(0.025, "in"), legend.position = "bottom", axis.ticks.y = element_blank(), axis.line = element_line(size = 0.2), axis.ticks = element_line(size = 0.2)) + 
            guides(size = guide_legend(title = "Percent Expressed"))

    if (return.plot){
        return(plot)
    } else {
        pdf(paste0(outputdir, file_name, "_DotVert.pdf"), width = 6 * width.scale, height = ceiling(height.unit * length(features)) + height.base, useDingbats = FALSE)
        print(plot)
        dev.off()
    }
}
...
plot_sp_heatmap <- function(avg, features, scale.exp = TRUE, min_max = c(-2.5, 2.5), file_name, cluster.order = cls_order, only.homo = TRUE, height.scale = 1, width.scale = 1, return.plot = FALSE, row.cex.sf = 1, col.cex.sf = 1) {
    ## Set the parameters
    ...
    htlist <- Heatmap(plot_data, name = "scaled_expr", 
                    ...
                    row_title_rot = 0, column_title = NULL, row_title_gp = gpar(fontsize = 12 * row.cex.sf), 
                    show_column_names = TRUE, show_row_names = FALSE, column_names_gp = gpar(fontsize = 10 * col.cex.sf),column_names_rot = 45, 
...
plot_sp_all <- function(object = object, resolution = c("mres", "hres")[1], features, only.homo = FALSE, file_name, ident.use = c("ExN", "InN", "NNC", "All", NULL)[4]) {
    ...
    if (resolution == "mres"){
        ...
        sf <- c(width = switch(ident.use, All = 1, ExN = 0.5, InN = 0.5, NNC = 0.5),
                height = switch(ident.use, All = 1, ExN = 0.5, InN = 0.5, NNC = 0.5),
                row = switch(ident.use, All = 1, ExN = 1, InN = 1, NNC = 1),
                col = switch(ident.use, All = 1, ExN = 1, InN = 1, NNC = 1),
                unit = switch(ident.use, All = 1, ExN = 1, InN = 1, NNC = 1),
                height.base = 1.5)
    } else if (resolution == "hres"){
        all_cls <- readRDS(file = paste0(dataDir, "PFC.cluster.order.rds"))
        cls_order <- switch(ident.use, 
                    All = all_cls, 
                    ExN = all_cls[c(1:14, 16:21, 15, 22:40)],
                    InN = all_cls[41:86],
                    NNC = all_cls[87:115]) 
        sf <- c(width = switch(ident.use, All = 3, ExN = 1.6, InN = 1.6, NNC = 1),
                height = switch(ident.use, All = 3, ExN = 1.6, InN = 1.6, NNC = 1),
                row = switch(ident.use, All = 0.8, ExN = 0.8, InN = 0.8, NNC = 0.8),
                col = switch(ident.use, All = 0.8, ExN = 0.8, InN = 0.8, NNC = 0.8),
                unit = switch(ident.use, All = 1, ExN = 1, InN = 1, NNC = 1),
                height.base = 2.5)
    } else {
    ...
    ## Subset cluster (homolog clusters?)
    if (only.homo){
        sp_cls <- c("L2-3 CUX2 ARHGAP18", "InN LAMP5 SYT10", "rAstro AQP4 OSMR","nOligo MOG CDH7", "Micro P2RY12 CCL3", "Micro P2RY12 GLDN", "B EBF1 IGKC", "ABC SLC47A1 SLC4A4", "cOPC PDGFRA TOP2A")
        cls_order <- setdiff(cls_order, sp_cls)
    }


    plot_sp_heatmap(avg = avgs, features = features, scale.exp = TRUE, min_max = c(-2.5, 2.5), file_name = file_name, cluster.order = cls_order, height.scale = 1 * sf["height"], width.scale = 1 * sf["width"], row.cex.sf = 1 * sf["row"], col.cex.sf = 1 * sf["col"], return.plot = FALSE)


    ## scaled by size
    CirclePlot.horizontal(avg = avgs, ratio = ratios, features = features, dot.min = 0, file_name = paste0(file_name, "_bysize"), dot.scale = 4, scale.by = "size", cluster.order = cls_order, stroke.size = 0, stroke.matrix = NULL, width.scale = 1 * sf["width"], height.base = sf["height.base"], row.cex.sf = 1 * sf["row"], col.cex.sf = 1 * sf["col"])
    CirclePlot.vertical(avg = avgs, ratio = ratios, features = features, dot.min = 0, file_name = paste0(file_name, "_bysize"), dot.scale = 4, scale.by = "size", cluster.order = cls_order, stroke.size = 0, stroke.matrix = NULL, width.scale = 1 * sf["width"], height.base = sf["height.base"], row.cex.sf = 1 * sf["row"], col.cex.sf = 1 * sf["col"])


    ## scaled by radius
    CirclePlot.horizontal(avg = avgs, ratio = ratios, features = features, dot.min = 0, file_name = paste0(file_name, "_radius"), dot.scale = 4, scale.by = "radius", cluster.order = cls_order, stroke.size = 0, stroke.matrix = NULL, width.scale = 1 * sf["width"], height.base = sf["height.base"], row.cex.sf = 1 * sf["row"], col.cex.sf = 1 * sf["col"])
    CirclePlot.vertical(avg = avgs, ratio = ratios, features = features, dot.min = 0, file_name = paste0(file_name, "_radius"), dot.scale = 4, scale.by = "radius", cluster.order = cls_order, stroke.size = 0, stroke.matrix = NULL, width.scale = 1 * sf["width"], height.base = sf["height.base"], row.cex.sf = 1 * sf["row"], col.cex.sf = 1 * sf["col"])
