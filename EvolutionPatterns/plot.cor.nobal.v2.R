...
avg <- readRDS(file = paste0(dataDir, "Expr_avg_ratio_by_species.rds"))[["fig1cluster"]]$avg
avg_hmf <- readRDS("../data/addon_human_MF_avg.rds")
colnames(avg_hmf) <- paste0("Human", "|", colnames(avg_hmf))
avg <- cbind(avg, avg_hmf[rownames(avg), , drop = FALSE])


## Get the highly variable genes
hvg_list <- readRDS(file = paste0(dataDir, "HVG.list.", "All", ".rds"))
...
hvg <- lapply(sp_list, function(x) 
        SelectHVG(hvg.list = hvg_list[x], nfeatures = 2500)
        ) %>%
            unlist() %>% 
            unique()
...
pdf(paste0(outputdir, "DIV_1minusCor_noBalanceCTP_combine_v2.pdf"), width = 10, height = 4)
