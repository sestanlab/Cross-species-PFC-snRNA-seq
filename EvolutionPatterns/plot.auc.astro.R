...
marlist <- readRDS(file = paste0(inputdir, "Astro.speccls.putative.markers.rds"))
...
seu_list <- lapply(all_sps, function(sp) {
	seu <- readRDS(file = paste0(inputdir, "Astro_insp_", sp, ".rds"))
...
seu_nlist <- lapply(all_sps, function(sp) {
	...
	obj <- seu_list[[sp]]
	aucsub <- GetModuleScore(assay.data = obj$RNA@data, features = marlist, method = "aucell", input_dir = inputdir, file_name = paste0("AstroMarkerAUC_", sp), output_dir = outputdir)$auc
...
anno_list <- lapply(all_sps, function(sp){
	    ...
	    p2 <- ggplot(sp_data, aes_string(x = "UMAP_1", y = "UMAP_2", color = "samplename")) +
				rasterise(geom_point(size = 0.5, shape = 16, alpha = 0.5), dpi = 300, scale = 1) +
	            theme_classic() + 
...
all_types <- c("Human rAstro", "Chimpanzee rAstro")
type_list <- list()
...
pdf(paste0(outputdir, "SF.validate.spec-clusters.rAstro.AUC.cbn.pdf"), width = 4 * (2 + length(all_types)), height = 4 * 4)
