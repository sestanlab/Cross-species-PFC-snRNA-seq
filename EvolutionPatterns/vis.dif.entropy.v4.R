...
pdata <- lapply(cls_ord, function(cls) {
	print(cls)
	res <- readRDS(file = paste0(inputdir, "UMAP_Bin_Res_v4/Entropy_list_regressed_", cls, ".v3.rds"))

	## Calculate mean entropy increases
	menp <- lapply(res, function(xx) {
		yy <- lapply(xx, function(mat) {
			enps <- apply(mat, 2, function(cc) sum(cc[cc > 0.5])) %>% .[all_sps]
			return(enps)
			}) %>%
			Reduce("+", .)
		yy <- yy/length(xx)
		yy
			...
			setNames(., paste0("s", 1:length(res))) %>%
			as.data.frame() %>%
			t() %>%
			as.matrix()
	...
	df <- menp %>%
			...
			tidyr::gather(., "species", "entropy", all_of(all_sps)) %>%
			mutate(cluster = cls)
...
pval_res <- PairWiseTest(data = pdata, group.by = "cluster2", split.by = "species", value.col = "entropy", p.adjust.method = "fdr", fc.thre = 0.15, pseudo.count = 1)
sig_df <- pval_res$df %>%
		...
		filter(nhits == 3) %>%
		ungroup() %>%
		mutate(label = "**", y = ceiling(max(pdata$entropy) * 1.06))

p1 <- ggplot(pdata, aes(x = cluster2, y = entropy)) +
...
pdf(paste0(outputdir, "Entropy.increase.v4.pdf"), width = 8, height = 2.3)
print(p1)
dev.off()



save(pdata, pval_res, sig_df, file = paste0(inputdir, "Entropy.increase.v4.Rdata"))
