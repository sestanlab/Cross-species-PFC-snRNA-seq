...
slim.dex <- all.dex %>%
					filter(pct.1 >= 0.25 & pct.2 <= 0.25 & ratio_fc >= 1.5 & avg_logFC >= log(2.5)) %>%
...
dex.up.top$Human <- c(dex.up.top$Human, "TWIST2")
...
source("../scripts/pfc.fun.R")
plot_dex_heatmap(data = data_use, split.by = "species", group.by = "cluster", label_genes = label_genes, label_col = label_cols, font_scale = 4, height_unit = 0.01, file_name = paste0("HPRC_DEX_", ctp), output_dir = outputdir, show_rownames = FALSE, color_limits = seq(-1, 2.5, 0.5), pdf_width = 18, pdf_height = 12, row_split = gene_split, heat_width = 6)
