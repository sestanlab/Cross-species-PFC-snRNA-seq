...
pfc <- readRDS(file = paste0(dataDir, "Final_PFC_HPRC.InN.07092021.rds"))
all_cls <- c("MGE", "CGE")
sp <- args[1]
ctp <- args[2]
...
for (smp in smps){
	...
	saveRDS(pfc_sub, file = paste0(inputdir, "ThreeDonorInte/", "ThreeSmpInte_", sp, "_", ctp, "_rm_", smp, ".rds"))
}
